...
if ($_POST['action']) {
    ...
    // check permission
    $perm->ExitIfNotAccess(array(NC_PERM_MODULE, 'filemanager'), 'file_property_edit');
    ...
    if (!$nc_filemanager->can_interact_with_file($absolute_path)) {
        echo json_encode(array('error' => NETCAT_MODERATION_ERROR_NORIGHTS), true);
        exit;
    }
